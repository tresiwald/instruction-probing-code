...
class LlamaBypassDecoderLayer(nn.Module):

    def __init__(self, layer):
        super().__init__()
        self.org_layer = layer
    ...
    def forward(
        ...
        hidden_states = self.org_layer.input_layernorm(hidden_states)

        # Self Attention
        hidden_states, self_attn_weights, present_key_value = self.org_layer.self_attn(
...
class MistralBypassDecoderLayer(nn.Module):
...
class Olmo2BypassDecoderLayer(nn.Module):
...
class OlmoBypassDecoderLayer(nn.Module):
    def __init__(self, layer):
        super().__init__()
        self.org_layer = layer
